...
async function checkLastQuery(conn, check, records) {
	...
	await doCheckLastQuery(conn, check, e3, true, records);
}

async function doCheckLastQuery(conn, check, e, positive, records) {
	if (!!check.not) {
		await doCheckLastQuery(conn, check.not, () => e().not, !positive);
	}
	if (!!check.equal_to) {
	...
	if (typeof check.len !== "undefined") {
		...
		if (isFinite(check.len)) {
			e().lengthOf(Number(check.len), lengthError(check.len, '', '', _`__NOT(PRE)__`, _`__NOT(POST)__`));
		}
		if (isFinite(check.len.least)) {
			e().lengthOf.least(Number(check.len.least), lengthError(check.len.least, _`__GE(PRE)__`, _`__GE(POST)__`, _`__LT(PRE)__`, _`__LT(POST)__`));
		}
		if (isFinite(check.len.most)) {
			e().lengthOf.most(Number(check.len.most), lengthError(check.len.most, _`__LE(PRE)__`, _`__LE(POST)__`, _`__GT(PRE)__`, _`__GT(POST)__`));
		}
	}
	if (!!check.contain) {
