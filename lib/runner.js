...
async function checkLastQuery(conn, check, records) {
	...
	if (!!check.columns) {
		e = e.columns(check.columns);
	}
	if (!!check.without) {
		e = e.without(check.without);
	}
	await doCheckLastQuery(conn, check, e)
}

async function doCheckLastQuery(conn, check, e) {
	if (!!check.not) {
		await doCheckLastQuery(conn, check.not, e.not);
		...
		for (let testcase of self.yaml.testcases) {
			describe("", function () {
				...
				it(title, async () => {
					let conn;
					try {
						conn = await Connection.new({ client: self.yaml.client, clean: true, connection: ':memory:' });
