...
async function checkLastQuery(conn, check, records, asTable) {
	...
	await doCheckLastQuery(conn, check, e, true, records, __(check.message));
}
...
async function checkError(conn, check) {
	function message(msg) {
		if (!!check.message) {
			...
			return msg + _`.` + ' ' + _`Expected error` + ': ' + __(`__sqle_${check.expected}__`);
		} else {
			return msg;
		}
	}
	...
	try {
		...
		const type = conn.errorOf(e);
		if (check.expected) {
			expect(type, message(_`Unexpected error detected`)).to.equal(check.expected);
		}
	}
}
