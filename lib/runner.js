		...
		await Promise.all(
			[precheck.not_empty].flat().map(async target => {
				const sqls = (await promisify(fs.readFile)(target, 'utf-8'))
					...
					.split(';')
					.filter(s => s.length > 0);
				const res = await conn.queryFromFile(target);
	...
	if (!!precheck.one_command) {
		await Promise.all(
			[precheck.one_command].flat().map(async target => {
				const results = await conn.queryFromFile(target);
				console.log(results);
			})
		);
	}
	...
	runAll() {
		...
		for (let testcase of self.yaml.testcases) {
			describe("", function () {
				...
				it(title, async () => {
					const conn = await Connection.new({ client: self.yaml.client, clean: true });
