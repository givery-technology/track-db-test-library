...
async function checkAutoIncrement(conn, check, expected) {
	...
	for (let i = 0; i < data.length; i++) {
		...
		if (i === data.length - 1) {
			expected = isDataString ? check.expected : check.data[check.data.length - 1];
			if (!!expected) {
				await checkLastQuery(conn, { equal_to: [expected].flat(), without: check.column }, [actual]);
			}
		}
	}
}
...
function preprocess(testcase) {
	...
	if (!!testcase.foreach) {
		return testcase.foreach.map(props => applyTemplate(buildProps(props, testcase.default), testcase.template));
	} else {
		return [applyTemplate(buildProps(), testcase)];
	}
}
			...
			it(__(testcase.title), async () => {
				async function doTest(tc) {
					...
					try {
						...
						let { records, sql } = (await conn.queryAll((tc.exec || []).flat())).slice(-1)[0];
						let asTable;

						if (tc.table && (typeof tc.table === 'string' || tc.table instanceof String)) {
							sql = '';
							records = await conn.tableSchema(tc.table);
							asTable = tc.table;
						}
						...
						for (let check of checks) {
							if (check.ecma) {
								...
								await checkAutoIncrement(conn, check.auto_increment, check.equal_to);
							} else if (check.error) {
