...
module.exports = function(chai, util) {
	...
	function assertRecordContain(val, msg) {
		...
		if (val instanceof Function) {
			...
			showRecord = true;
			fn = record => equal(record, val);
		}
		const ok = obj.some(fn);

		this.assert(
			ok,
			showRecord ? [
				!!msg ? '' : _`Records should contain the following record`,
				_`Expected:`,
				indent(records.format([val])),
				_`Actual:`,
			...
			showRecord ? [
				!!msg ? '' : _`Records should not contain the following record`,
				_`Expected:`,
				indent(records.format([val])),
				_`Actual:`,
				indent(records.format(obj)),
