const formatRecords = require('./format.js').records;
...
module.exports = function(chai, util) {
	...
	function assertFullscan(tables, msg) {
		...
		const records = flag(this, 'object');
		const every = !!flag(this, 'all');
		this.assert(
			fullscan(records, tables, every),
			_`expected table full scan detected${'\n' + formatRecords(records)}`,
			_`expected table full scan not detected${'\n' + formatRecords(records)}`,
		);
	}
