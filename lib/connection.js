...
class Connection {
	...
	async query(sql, opt_args) {
		function rows(records) { !!records.rows ? records.rows /* postgres */ : records /* sqlite */};
		if (sql.trim().length === 0) {
			throw 'Empty query';
		}
		const count = rows(await this._conn.raw(`SELECT count(1) AS count FROM (${sql}) AS x`, opt_args)).length;
