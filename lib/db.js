...
class Connection {
	...
	async query(sql, opt_args) {
		...
		return await this.conn.raw(sql, opt_args);
	}
	...
	async loadFromCSV(path, table) {
		...
		const rs = await Promise.all(
			bulks.map(bulk => this.conn.insert(bulk).into(table))
		);
		return flatten(rs);
	}
	...
	async dryrun(fn) {
		try {
			return await this.conn.transaction(tx =>
				Promise.resolve(tx).then(fn).then(tx.rollback, tx.rollback)
			);
		} catch (result) {
